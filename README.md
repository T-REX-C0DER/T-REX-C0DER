<p align="center">
<img width="90%" src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/colored.png">
</p>

<h3>AI • Data Science • Intelligent Systems Engineering</h3>

<p>
🚀 AI & Data Science Engineer in the making <br>
🤖 Machine Learning • 🧠 Intelligent Systems • 🔐 Cyber • ⚛️ Quantum
</p>

<br>

<img src="https://readme-typing-svg.herokuapp.com?size=22&duration=2300&pause=700&color=C0C0C0&center=true&vCenter=true&width=720&lines=Engineering+Real-World+Intelligent+Systems;AI+%7C+Data+Science+%7C+System+Design;Liquid-Metal+Mindset+%7C+Solid+Fundamentals;Learning+Deep.+Building+Hard.+Thinking+Long-Term."/>

<br><br>

</div>

---

## 🧠 About Me

AI & Data Science engineering student focused on **engineering real systems**, not just writing scripts.

I work toward:
- Designing **intelligent, data-driven platforms**
- Applying **machine learning to real operational problems**
- Building **secure, scalable system architectures**
- Exploring **quantum computation foundations**

I prioritize **depth over noise** — fundamentals, system thinking, and long-term engineering growth.

---

<br>

## 🧬 Engineering Stack

<br>

<table align="center">
<tr>
<td align="center" width="33%">
<h3>🧠 AI / Core</h3>
<img src="https://skillicons.dev/icons?i=python,java,c,cpp" />
<br><br>
Machine Learning • DSA • Algorithms
</td>

<td align="center" width="33%">
<h3>🌐 Systems / Web</h3>
<img src="https://skillicons.dev/icons?i=html,css,js,nodejs" />
<br><br>
Backend Logic • APIs • Full-Stack Foundations
</td>

<td align="center" width="33%">
<h3>🗄️ Data / Tools</h3>
<img src="https://skillicons.dev/icons?i=mongodb,mysql,git,github,linux" />
<br><br>
Databases • Version Control • OS Fundamentals
</td>
</tr>
</table>

---

<br>

## 🏗️ Flagship Projects

🚀 **Society Management System**  
→ Real-world data platform for records, maintenance workflows & structured feedback  

🧠 **SkillSight (Hackathon)**  
→ AI-based skill verification and assessment system  

📊 **Machine Learning Lab**  
→ Predictive models, data analysis pipelines & ML experiments  

⚠️ Private repositories are intentional. System design, documentation, and architecture matter more than exposed code.

---

<br>

## 📊 Engineering Activity

<br>

<p align="center">
<img width="80%" src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=T-REX-C0DER&theme=tokyonight"/>
</p>

---

<br>

## 🧩 How I Think (Engineering Philosophy)

• Systems > scripts  
• Fundamentals before frameworks  
• Build → test → break → redesign  
• Long-term skill compounding over short-term hype  
• Real problems > tutorial projects  

---

<br>

## 🌱 Current Focus

- Advanced Data Structures & Algorithms  
- Machine Learning engineering foundations  
- Secure system design principles  
- Quantum computing basics  
- Long-term scalable projects  

---

<br>

## 🧠 Developer Quote

<br>

<p align="center">
<img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=radical"/>
</p>

---

<br><br>

<div align="center">

⭐ Engineering intelligent systems.  
📬 Open to collaborations, research, and meaningful technology.

</div>

<br>

<!-- ========== FOOTER : LIQUID METAL CLOSING ========== -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=venom&color=0:000000,45:1A1A1A,75:4F4F4F,100:C0C0C0&height=140&section=footer&animation=twinkling"/>

